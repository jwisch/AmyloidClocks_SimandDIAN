...
all_files <- all_files[all_files %like% "RMSE"]
...
df_all <- map_dfr(files, function(f) {
  ...
  matches <- str_match(
    fname,
    "alpha_([-0-9.]+)_omega_([-0-9.]+)_xi_([-0-9.]+)RMSE\\.RDS$"
  )
...
dt_summary <- data.frame(dt[
  ...
  .(
    ...
    lower_2.5  = quantile(RMSE, 0.025, na.rm = TRUE),
    upper_97.5 = quantile(RMSE, 0.975, na.rm = TRUE)
  ),
  by = .(alpha, omega, xi)
])


p_RMSE <- ggplot(dt_summary[dt_summary$xi %in% c(-1, 0, 1) &
                             dt_summary$alpha %in% c(-2, -1, 0, 1, 2),], 
                aes(x = factor(alpha), 
                    y = median_RMSE, 
                    ymin = lower_2.5, 
...
grid.arrange(p, as.grob(p_MAE), as.grob(p_RMSE), nrow = 3, ncol = 1)

graph2ppt(file = "./Figures/SuppFig1.pptx", width = 8, height = 8.5)


#Anova to figure out relative contribution of xi, omega, alpha
