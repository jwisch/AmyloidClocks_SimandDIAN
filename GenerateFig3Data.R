###This bootstrapped code simulates repeated disease time measurements
##With varying within-individual variation
##The other parameters correspond with pib
#Define the other skew-normal parameters here
xi <- 1
omega <- 0.5
alpha <- 0

filename <- "Xi1Omegapoint5Alpha0"
# --- libraries ---
library(mgcv)
library(dplyr)
library(ggplot2)
...
mean_within_above_vec <- c( 0.03, 0.07, 0.1, 0.15, 0.2)
n_ids <- 300
...
for(i in 1:length(mean_within_above_vec)){
  mean_within_above <- mean_within_above_vec[i]
...
ggplot(tmp[[1]], aes(x = TimefromBaseline, y = Z_true, group = ID)) + geom_line(colour = "grey80") + theme_bw() +
  geom_line(data = tmp[[1]][tmp[[1]]$ID == "1",], aes(x = TimefromBaseline, y = Z_true), colour = "black", size = 1.4) + 
  xlab("Time from Baseline") + ylab("Z Score") + ggtitle("Individual Rates")
...
saveRDS(df_result, paste0("./RDS_Simulation_v02_fullBS/", "df_result_", filename,
                          "_mean_within_above_",
                          mean_within_above,".RDS"))
saveRDS(df_interp, paste0("./RDS_Simulation_v02_fullBS/", "df_interp_", filename, "_mean_within_above_",
                          mean_within_above,".RDS"))

saveRDS(MAE, paste0("./RDS_Simulation_v02_fullBS/", "MAE_", filename,"_mean_within_above_",
                    mean_within_above, ".RDS"))

saveRDS(RMSE, paste0("./RDS_Simulation_v02_fullBS/", "RMSE_", filename,"_mean_within_above_",
                     mean_within_above, ".RDS"))

saveRDS(R2, paste0("./RDS_Simulation_v02_fullBS/", "R2_", filename,"_mean_within_above_",
                   mean_within_above, ".RDS"))
